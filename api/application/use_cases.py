...
from domain.repositories.ChemicalsAnalizedRepository import ChemicalsAnalizedRepository
...
def get_chemicals_analized_data_googlesheet():
    from main import app
    sheet_name = 'QuimicosAnalisados'
    gc_adapter = GoogleSheetAdapter(app.config['CREDENTIALS_GOOGLE_SHEET'], app.config['SHEET_ID'])    
    chemicals_analized_repository = ChemicalsAnalizedRepository(gc_adapter.connect_sheets(), sheet_name)
    return chemicals_analized_repository.work_sheet().get_all_records()

def get_chemicals_analized_data_database():
    from main import app    
    database_uri = app.config['SQLALCHEMY_DATABASE_URI']
    database_adapter = DatabaseAdapter(database_uri)
    chemicals_analized_repository = ChemicalsAnalizedRepository(database_adapter=database_adapter)
    return chemicals_analized_repository.get_chemicals_analized()
...
def insert_chemicals_analized_data(data):
    from main import app    
    database_uri = app.config['SQLALCHEMY_DATABASE_URI']
    database_adapter = DatabaseAdapter(database_uri)
    status=ChemicalsAnalizedRepository(database_adapter=database_adapter).insert_chemicals_analized(data)
    return status
...
def update_chemicals_analized_data(data):
    from main import app    
    database_uri = app.config['SQLALCHEMY_DATABASE_URI']
    database_adapter = DatabaseAdapter(database_uri)
